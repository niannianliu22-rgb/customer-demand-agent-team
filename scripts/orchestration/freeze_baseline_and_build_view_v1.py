...
RUN_ID='RUN-202608-TEAM-20260823T185333'; RUN=ROOT/'runs'/RUN_ID
...
def rel(p): return str(p.relative_to(RUN))
...
def main():
 state=j(RUN/'state/runtime_state.json'); manifest=j(RUN/'run_manifest.json'); manifest.update({'baseline_status':'FROZEN','baseline_type':'FIRST_FULL_E2E_PASS','baseline_manifest':'baseline/baseline_manifest.json'}); dump(RUN/'run_manifest.json',manifest); dispatch=yaml.safe_load((ROOT/'config/orchestration/agent_dispatch_registry_v1.yaml').read_text())['agents']; routing=yaml.safe_load((RUN/'snapshots/model_routing.yaml').read_text())['routes']; binding=yaml.safe_load((RUN/'snapshots/model_runtime_binding.yaml').read_text())['bindings']; gov=yaml.safe_load((RUN/'snapshots/agent_governance.yaml').read_text())
 ...
 dump(view/'04_gate_status.json',{'gates':[{**j(g),'path':rel(g)} for g in gates]})
